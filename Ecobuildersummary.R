...
library(dplyr)
library(tidyverse)
load('research_20nodes.RData')
load('researchworld_binary_matrix.RData')
#web.name
level1_summary <- as.data.frame(unique(level1_20nodes$foodweb))

#num.sp
num.sp <- table(level1_20nodes$foodweb)[[1]]
for(i in 2:24){
  num.sp <- c(num.sp,table(level1_20nodes$foodweb)[[i]])
  num.sp
}
level1_summary[,2] <- num.sp

#num.B
datanum.B <- filter(level1_20nodes,b<0)
...
for(i in level1_summary$`unique(level1_20nodes$foodweb)`){
  count <- 0
...
list.B.num <- list()
for(k in unique(level1_20nodes$foodweb)){
  B.num <- c()
...
#mean.mass
mean.mass <- aggregate(level1_20nodes$mass,by=list(foodweb=level1_20nodes$foodweb),mean)[,2]
...
level1_foodwebnum <- unique(level1_20nodes$foodweb)
  ...
  for(i in 1:sqrt(length(level1_binary_matrix[[level1_foodwebnum[k]]]))){
    for(j in 1:sqrt(length(level1_binary_matrix[[level1_foodwebnum[k]]]))){
      if(level1_binary_matrix[[level1_foodwebnum[k]]][i,j]==1){
        con.mass <- c(con.mass,level1_20nodes$mass[j+number[k]])
        res.mass <- c(res.mass,level1_20nodes$mass[i+number[k]])
      }
    }
  }
}
...
#level2
#web.name
level2_summary <- as.data.frame(unique(level2_20nodes$foodweb))

#num.sp
num.sp <- table(level2_20nodes$foodweb)[[1]]
for(i in 2:180){
  num.sp <- c(num.sp,table(level2_20nodes$foodweb)[[i]])
  num.sp
}
level2_summary[,2] <- num.sp

#num.B
datanum.B <- filter(level2_20nodes,b<0)
num.B <- table(datanum.B$foodweb)[[1]]
for(i in 2:180){
  num.B <- c(num.B,table(datanum.B$foodweb)[[i]])
  num.B
...
#num.T
a <- list()
for(i in level2_summary$`unique(level2_20nodes$foodweb)`){
  count <- 0
...
#get B.num, T.num, I.num
list.B.num <- list()
for(k in unique(level2_20nodes$foodweb)){
  B.num <- c()
...
#mean.mass
mean.mass <- aggregate(level2_20nodes$mass,by=list(foodweb=level2_20nodes$foodweb),mean)[,2]
...
level2_foodwebnum <- unique(level2_20nodes$foodweb)
  ...
  for(i in 1:sqrt(length(level2_binary_matrix[[(level2_foodwebnum[k])-397]]))){
    for(j in 1:sqrt(length(level2_binary_matrix[[(level2_foodwebnum[k])-397]]))){
      if(level2_binary_matrix[[(level2_foodwebnum[k])-397]][i,j]==1){
        con.mass <- c(con.mass,level2_20nodes$mass[j+number[k]])
        res.mass <- c(res.mass,level2_20nodes$mass[i+number[k]])
      }
    }
  }
}
...
#level3
#web.name
level3_summary <- as.data.frame(unique(level3_20nodes$foodweb))

#num.sp
num.sp <- table(level3_20nodes$foodweb)[[1]]
for(i in 2:32){
  num.sp <- c(num.sp,table(level3_20nodes$foodweb)[[i]])
  num.sp
}
level3_summary[,2] <- num.sp

#num.B
datanum.B <- filter(level3_20nodes,b<0)
num.B <- table(datanum.B$foodweb)[[1]]
for(i in 2:32){
  num.B <- c(num.B,table(datanum.B$foodweb)[[i]])
  num.B
...
#num.T
a <- list()
for(i in level3_summary$`unique(level3_20nodes$foodweb)`){
  count <- 0
...
#get B.num, T.num, I.num
list.B.num <- list()
for(k in unique(level3_20nodes$foodweb)){
  B.num <- c()
...
#mean.mass
mean.mass <- aggregate(level3_20nodes$mass,by=list(foodweb=level3_20nodes$foodweb),mean)[,2]
...
level3_foodwebnum <- unique(level3_20nodes$foodweb)
  ...
  for(i in 1:sqrt(length(level3_binary_matrix[[(level3_foodwebnum[k])-679]]))){
    for(j in 1:sqrt(length(level3_binary_matrix[[(level3_foodwebnum[k])-679]]))){
      if(level3_binary_matrix[[(level3_foodwebnum[k])-679]][i,j]==1){
        con.mass <- c(con.mass,level3_20nodes$mass[j+number[k]])
        res.mass <- c(res.mass,level3_20nodes$mass[i+number[k]])
      }
    }
  }
}
...
save(level1_summary,level2_summary,level3_summary,file='EcoBuildersummary.RData')
